...
echo "2. Segment node have to be expanded by 4 units."
...
read -p "1. What is the segment node's prefix_name in your segment node? : " SEGPREFIX
read -p "2. How many zeros are in the segment node hostname? [ 0 | 1 | 2 ] : " ZEROCNT
...
read -p "3. What is the instance's prefix_name in your cluster? : " INSTPREFIX
read -p "4. What is segment node number of the first expand nodes? : " SCNT
read -p "5. What is segment node number of the last expand nodes? : " ECNT
SNUM=`expr ${SCNT} / 4`
ENUM=`expr \( ${ECNT} - 1 \) / 4`
