...
## explain
## MAXCONTENT : Number of primary instances in four segments (start 0, 16 = seg_nodes 4 * instances per seg_nodes 4)
## SEGPREFIX  : segment node prefix name setting
## INSTPREFIX : segment instance prefix name setting
## SNUM/ENUM  : Number of quater groups to add (quater = 4 seg_nodes, start 0, if 2nd fullrack = (4,7)
...
read -p "What is the segment node's prefix_name in your segment node? : " SEGPREFIX
...
read -p "What is the instance's prefix_name in your cluster? : " INSTPREFIX
read -p "What is segment node number of the first expand nodes? : " SCNT
read -p "What is segment node number of the last expand nodes? : " ECNT
SNUM=`expr ${SCNT} / 4`
ENUM=`expr ${ECNT} / 4`
...
(
       SELECT sc.dbid::int,
              ...
              split_part(sc.datadir, '${INSTPRIFIX}', 1)    AS location,
              io.prefix_name,
              io.nodes,
              io.instances
...
FROM   (
                SELECT   tm.dbid    + instances * nodes * 2 * i AS dbid,
                         ...
                         tm.location || '${INSTPRIFIX}' || (tm.content + instances * nodes * i) AS location
